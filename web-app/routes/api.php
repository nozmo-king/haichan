...
// DEVELOPMENT ENDPOINT SECURED - Only available in debug mode
if (config('app.debug') && app()->environment('local')) {
    Route::post('/dev/add-public-key', function (Illuminate\Http\Request $request) {
        $request->validate([
            'public_key' => 'required|string|size:66',
            'admin_token' => 'required|string', // Additional security requirement
        ]);

        // Verify admin token
        if ($request->admin_token !== config('app.key')) {

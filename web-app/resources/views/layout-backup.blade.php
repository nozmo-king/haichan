...
<body data-theme="classic">
    ...
    <script>
// Global utility functions for forms and replies
function toggleReplyForm(postId, type) {
    const formId = type === 'post' ? `reply-form-post-${postId}` : `reply-form-thread-${postId}`;
    const form = document.getElementById(formId);
    
    if (form) {
        if (form.style.display === 'none' || form.style.display === '') {
            form.style.display = 'block';
            // Initialize the miner for this specific post if it exists
            const minerKey = `replyMiner${postId}`;
            if (window[minerKey] && typeof window[minerKey].init === 'function') {
                window[minerKey].init();
            }
        } else {
...
function toggleImageSize(img) {
    if (img.style.maxWidth === 'none' || img.style.maxWidth === '') {
        img.style.maxWidth = img.dataset.originalMaxWidth || '150px';
        img.style.maxHeight = img.dataset.originalMaxHeight || '150px';
    } else {
...
function previewReplyImage(input, postId) {
    const preview = document.getElementById(`reply-preview-${postId}`);
    const img = document.getElementById(`reply-preview-img-${postId}`);
    const info = document.getElementById(`reply-file-info-${postId}`);
